...
CREATE TABLE upCategory (
	upcg_code number(8)    NOT NULL, -- 상위 카테고리코드
	upcg_name varchar2(30) NULL      -- 상위 카테고리명
);

-- 상위 카테고리 기본키
CREATE UNIQUE INDEX PK_upCategory
	ON upCategory ( -- 상위 카테고리
		upcg_code ASC -- 상위 카테고리코드
	);
...
CREATE TABLE downCategory (
	upcg_code   number(8)    NOT NULL, -- 상위 카테고리코드
	downcg_code number(8)    NOT NULL, -- 하위 카테고리코드
...
CREATE TABLE Products (
	...
	pdate       DATE           NULL,     -- 등록일
	upcg_code   number(8)      NULL,     -- 상위 카테고리코드
	downcg_code number(8)      NULL      -- 하위 카테고리코드
);
